...
export function TaskChecklist() {
  const data = [{name:'Sarah Jenkins', hr:'Scheduled', mgr:'Pending', team:'Pending', status:'Pending'},{name:'Jane Smith', hr:'Done', mgr:'Done', team:'Done', status:'Completed'}];
    ...
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Departmental and HR onboarding task progress</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Employee</th><th className='px-6 py-3'>HR Orientation</th><th className='px-6 py-3'>Manager Sync</th><th className='px-6 py-3'>Team Intro</th><th className='px-6 py-3'>Overall Status</th>
