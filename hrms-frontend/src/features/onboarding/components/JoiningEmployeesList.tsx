...
export function JoiningEmployeesList() {
  const data = [{name:'Sarah Jenkins', role:'Senior Developer', dept:'Engineering', date:'2026-09-01', status:'Pending'},{name:'Michael Chen', role:'Product Manager', dept:'Product', date:'2026-09-05', status:'Completed'}];

  return (
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <CardTitle className="text-lg font-semibold text-slate-800">Joining Employees</CardTitle>
          <p className="text-sm text-slate-500 mt-1">Manage all upcoming new hires</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Employee Name</th><th className='px-6 py-3'>Role</th><th className='px-6 py-3'>Department</th><th className='px-6 py-3'>Joining Date</th><th className='px-6 py-3'>Status</th>
