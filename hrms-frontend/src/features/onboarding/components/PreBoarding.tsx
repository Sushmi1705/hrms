...
export function PreBoarding() {
  const data = [{name:'Sarah Jenkins', email:'Sent', forms:'3/5', access:'Active', status:'In Progress'},{name:'David Lee', email:'Sent', forms:'5/5', access:'Active', status:'Completed'}];

  return (
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <CardTitle className="text-lg font-semibold text-slate-800">Pre-Boarding Portal</CardTitle>
          <p className="text-sm text-slate-500 mt-1">Track pre-boarding activities and engagements</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Candidate</th><th className='px-6 py-3'>Welcome Email</th><th className='px-6 py-3'>Forms Submitted</th><th className='px-6 py-3'>Portal Access</th><th className='px-6 py-3'>Status</th>
