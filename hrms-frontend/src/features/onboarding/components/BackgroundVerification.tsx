...
export function BackgroundVerification() {
  const data = [{name:'Sarah Jenkins', agency:'FirstAdvantage', criminal:'Clear', emp:'Pending', status:'In Progress'},{name:'Tom Hanks', agency:'Checkr', criminal:'Clear', emp:'Clear', status:'Completed'}];

  return (
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <CardTitle className="text-lg font-semibold text-slate-800">Background Checks</CardTitle>
          <p className="text-sm text-slate-500 mt-1">Monitor third-party background verification status</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Candidate</th><th className='px-6 py-3'>Agency</th><th className='px-6 py-3'>Criminal Check</th><th className='px-6 py-3'>Employment Check</th><th className='px-6 py-3'>Status</th>
