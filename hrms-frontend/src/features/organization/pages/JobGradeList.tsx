...
import { getJobGrades, deleteJobGrade, JobGradeDto } from '../api/JobGradeApi';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Plus, Search, Filter, Download, MoreHorizontal, FileEdit, Trash2, LayoutGrid, Loader2 } from 'lucide-react';

export function JobGradeList() {
  const queryClient = useQueryClient();
  const { data, isLoading, isError } = useQuery({ queryKey: ['JobGrades'], queryFn: getJobGrades });
  const [search, setSearch] = useState('');

  const deleteMutation = useMutation({
    mutationFn: deleteJobGrade,
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['JobGrades'] })
  });
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">JobGrades</h1>
