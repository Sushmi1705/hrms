...
import { getCompanys, deleteCompany, CompanyDto } from '../api/CompanyApi';
...
export function CompanyList() {
  ...
  const { data, isLoading, isError } = useQuery({ queryKey: ['Companys'], queryFn: getCompanys });
  const [search, setSearch] = useState('');

  const deleteMutation = useMutation({
    mutationFn: deleteCompany,
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['Companys'] })
  });
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Companys</h1>
