...
import { getBranchs, deleteBranch, BranchDto } from '../api/BranchApi';
...
export function BranchList() {
  ...
  const { data, isLoading, isError } = useQuery({ queryKey: ['Branchs'], queryFn: getBranchs });
  const [search, setSearch] = useState('');

  const deleteMutation = useMutation({
    mutationFn: deleteBranch,
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['Branchs'] })
  });
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Branchs</h1>
