...
export function Delegation() {
  const data = [{dr:'Sarah Jenkins', de:'Bob Smith', sd:'2026-09-01', ed:'2026-09-15', status:'Active'},{dr:'Michael Chen', de:'Alice Cooper', sd:'2026-07-01', ed:'2026-07-10', status:'Expired'}];

  return (
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <CardTitle className="text-lg font-semibold text-slate-800">Approval Delegation</CardTitle>
          <p className="text-sm text-slate-500 mt-1">Temporary reassignment of approval rights</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Delegator</th><th className='px-6 py-3'>Delegatee</th><th className='px-6 py-3'>Start Date</th><th className='px-6 py-3'>End Date</th><th className='px-6 py-3'>Status</th>
