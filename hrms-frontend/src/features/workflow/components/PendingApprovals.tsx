...
export function PendingApprovals() {
  const data = [{id:'REQ-2026-089', req:'John Smith', mod:'Leave', step:'Manager Approval', status:'Pending'},{id:'REQ-2026-092', req:'Sarah Jenkins', mod:'Payroll', step:'Finance Verification', status:'Pending'},{id:'REQ-2026-104', req:'Michael Chen', mod:'Recruitment', step:'HR Head Approval', status:'Escalated'}];

  return (
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <CardTitle className="text-lg font-semibold text-slate-800">Universal Universal Inbox</CardTitle>
          <p className="text-sm text-slate-500 mt-1">All pending requests requiring action</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Request ID</th><th className='px-6 py-3'>Requester</th><th className='px-6 py-3'>Module</th><th className='px-6 py-3'>Current Step</th><th className='px-6 py-3'>Status</th>
                ...
                <tr key={i} className="hover:bg-slate-50 transition-colors">
                  {Object.values(item).map((val, j) => (
                    <td key={j} className="px-6 py-4 whitespace-nowrap">
                      {val === 'Active' || val === 'Approved' || val === 'Published' || val === 'Completed' ? (
                      ...
                      ) : val === 'Draft' || val === 'Pending' || val === 'In Progress' ? (
                      ...
                      ) : val === 'Archived' || val === 'Rejected' || val === 'Escalated' || val === 'Failed' || val === 'Expired' ? (
