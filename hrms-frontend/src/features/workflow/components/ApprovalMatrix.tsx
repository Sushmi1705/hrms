...
export function ApprovalMatrix() {
  const data = [{name:'Standard Expense', t1:'Direct Manager', t2:'Finance Dept', t3:'--', status:'Active'},{name:'Promotion Workflow', t1:'Manager', t2:'Dept Head', t3:'HR Director', status:'Active'}];

  return (
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <CardTitle className="text-lg font-semibold text-slate-800">Role-Based Approval Matrix</CardTitle>
          <p className="text-sm text-slate-500 mt-1">Hierarchical approval definitions</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Matrix Name</th><th className='px-6 py-3'>Tier 1</th><th className='px-6 py-3'>Tier 2</th><th className='px-6 py-3'>Tier 3</th><th className='px-6 py-3'>Status</th>
