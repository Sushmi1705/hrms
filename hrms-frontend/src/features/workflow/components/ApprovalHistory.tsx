...
export function ApprovalHistory() {
  const data = [{id:'REQ-2026-045', mod:'Leave', app:'David Lee', act:'Approved', date:'2026-08-26'},{id:'REQ-2026-042', mod:'Assets', app:'IT Admin', act:'Rejected', date:'2026-08-25'}];

  return (
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <CardTitle className="text-lg font-semibold text-slate-800">Complete Audit Trail</CardTitle>
          <p className="text-sm text-slate-500 mt-1">Historical record of all workflow actions</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Request ID</th><th className='px-6 py-3'>Module</th><th className='px-6 py-3'>Approver</th><th className='px-6 py-3'>Action Taken</th><th className='px-6 py-3'>Date</th>
