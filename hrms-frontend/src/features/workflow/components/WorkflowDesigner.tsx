...
export function WorkflowDesigner() {
  const data = [{name:'Standard Leave Request', mod:'Leave', nodes:'5', date:'2026-08-25', status:'Active'},{name:'Expense Reimbursement', mod:'Payroll', nodes:'7', date:'2026-08-20', status:'Active'},{name:'Employee Onboarding', mod:'Onboarding', nodes:'15', date:'2026-08-27', status:'Draft'}];

  return (
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <CardTitle className="text-lg font-semibold text-slate-800">Visual Workflow Designer</CardTitle>
          <p className="text-sm text-slate-500 mt-1">Drag-and-drop workflow builder</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Workflow Name</th><th className='px-6 py-3'>Module</th><th className='px-6 py-3'>Nodes</th><th className='px-6 py-3'>Last Updated</th><th className='px-6 py-3'>Status</th>
