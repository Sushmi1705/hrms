...
export function ApprovalRules() {
  const data = [{name:'High Value Expense', mod:'Payroll', cond:'Amount > ', pri:'High', status:'Active'},{name:'Extended Leave', mod:'Leave', cond:'Days > 14', pri:'Medium', status:'Active'}];

  return (
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <CardTitle className="text-lg font-semibold text-slate-800">Dynamic Approval Rules</CardTitle>
          <p className="text-sm text-slate-500 mt-1">Condition-based routing logic</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Rule Name</th><th className='px-6 py-3'>Module</th><th className='px-6 py-3'>Conditions</th><th className='px-6 py-3'>Priority</th><th className='px-6 py-3'>Status</th>
