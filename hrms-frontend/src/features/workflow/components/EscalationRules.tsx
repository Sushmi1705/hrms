...
export function EscalationRules() {
  const data = [{name:'Standard SLA', mod:'All', thr:'48 Hours', esc:'Next Level Manager', status:'Active'},{name:'Urgent Leave SLA', mod:'Leave', thr:'24 Hours', esc:'HR Partner', status:'Active'}];

  return (
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <CardTitle className="text-lg font-semibold text-slate-800">SLA & Escalation Policies</CardTitle>
          <p className="text-sm text-slate-500 mt-1">Automated escalation configurations</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Policy Name</th><th className='px-6 py-3'>Module</th><th className='px-6 py-3'>SLA Threshold</th><th className='px-6 py-3'>Escalate To</th><th className='px-6 py-3'>Status</th>
