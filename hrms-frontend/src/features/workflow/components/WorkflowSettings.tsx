...
export function WorkflowSettings() {
  const data = [{name:'Enable Email Notifications', cat:'Notifications', val:'True', date:'2026-01-01', status:'Active'},{name:'Default SLA Hours', cat:'SLA', val:'48', date:'2026-01-01', status:'Active'}];

  return (
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <CardTitle className="text-lg font-semibold text-slate-800">Engine Configuration</CardTitle>
          <p className="text-sm text-slate-500 mt-1">Global workflow engine parameters</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Setting Name</th><th className='px-6 py-3'>Category</th><th className='px-6 py-3'>Value</th><th className='px-6 py-3'>Last Updated</th><th className='px-6 py-3'>Status</th>
