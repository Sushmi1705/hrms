...
export function ClassroomTraining() {
  const data = [{name:'Agile Methodologies', inst:'Bob Smith', date:'2026-09-10 10:00 AM', cap:'24/30', status:'Scheduled'},{name:'Conflict Resolution', inst:'Dr. Alice Cooper', date:'2026-08-20 02:00 PM', cap:'30/30', status:'Completed'}];

  return (
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <CardTitle className="text-lg font-semibold text-slate-800">Classroom Sessions</CardTitle>
          <p className="text-sm text-slate-500 mt-1">Schedule and manage physical/virtual instructor-led sessions</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Session Name</th><th className='px-6 py-3'>Instructor</th><th className='px-6 py-3'>Date & Time</th><th className='px-6 py-3'>Capacity</th><th className='px-6 py-3'>Status</th>
