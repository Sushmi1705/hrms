...
export function Certifications() {
  const data = [{name:'Sarah Jenkins', cert:'AWS Certified Pro', issue:'2025-01-15', exp:'2028-01-15', status:'Certified'},{name:'David Lee', cert:'First Aid CPR', issue:'2023-05-10', exp:'2025-05-10', status:'Expired'}];

  return (
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <CardTitle className="text-lg font-semibold text-slate-800">Employee Certifications</CardTitle>
          <p className="text-sm text-slate-500 mt-1">Track issued certificates and expiries</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Employee Name</th><th className='px-6 py-3'>Certificate</th><th className='px-6 py-3'>Issue Date</th><th className='px-6 py-3'>Expiry Date</th><th className='px-6 py-3'>Status</th>
