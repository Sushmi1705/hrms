...
export function EmployeeLearning() {
  const data = [{name:'Sarah Jenkins', dept:'Engineering', comp:'12', hrs:'45', status:'Active'},{name:'Michael Chen', dept:'Product', comp:'4', hrs:'12', status:'Active'}];

  return (
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <CardTitle className="text-lg font-semibold text-slate-800">Employee Learning Profiles</CardTitle>
          <p className="text-sm text-slate-500 mt-1">Monitor individual employee training progress</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Employee Name</th><th className='px-6 py-3'>Department</th><th className='px-6 py-3'>Courses Completed</th><th className='px-6 py-3'>Training Hours</th><th className='px-6 py-3'>Status</th>
