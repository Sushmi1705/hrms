...
export function LMSSettings() {
  const data = [{name:'Auto-Assign Compliance', mod:'Automation', val:'Enabled', date:'2026-01-15', status:'Active'},{name:'Max Retakes Allowed', mod:'Assessments', val:'3', date:'2026-05-20', status:'Active'}];

  return (
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <CardTitle className="text-lg font-semibold text-slate-800">LMS Configuration</CardTitle>
          <p className="text-sm text-slate-500 mt-1">Configure learning platform parameters</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Configuration Setting</th><th className='px-6 py-3'>Module</th><th className='px-6 py-3'>Value</th><th className='px-6 py-3'>Last Updated</th><th className='px-6 py-3'>Status</th>
