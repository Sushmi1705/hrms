...
export function LearningPaths() {
  const data = [{name:'Full Stack Onboarding', tgt:'Engineering', crs:'5', prog:'65%', status:'Active'},{name:'Sales Bootcamp', tgt:'Sales', crs:'3', prog:'82%', status:'Active'}];
    ...
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Curated sequences of courses for role-based training</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Path Name</th><th className='px-6 py-3'>Assigned To</th><th className='px-6 py-3'>Courses Included</th><th className='px-6 py-3'>Average Progress</th><th className='px-6 py-3'>Status</th>
