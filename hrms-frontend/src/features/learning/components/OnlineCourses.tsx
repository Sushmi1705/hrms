...
export function OnlineCourses() {
  const data = [{title:'React Context API', fmt:'Video (MP4)', size:'250 MB', views:'430', status:'Published'},{title:'Compliance Deck', fmt:'SCORM 1.2', size:'45 MB', views:'1200', status:'Published'}];

  return (
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <CardTitle className="text-lg font-semibold text-slate-800">E-Learning SCORM & Videos</CardTitle>
          <p className="text-sm text-slate-500 mt-1">Manage digital course content and assets</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Content Title</th><th className='px-6 py-3'>Format</th><th className='px-6 py-3'>File Size</th><th className='px-6 py-3'>Views</th><th className='px-6 py-3'>Status</th>
