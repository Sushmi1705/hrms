...
export function FinalSettlement() {
  const data = [{name:'Alex Turner', leave:',200', dues:'-', total:',400', status:'Pending'},{name:'Chris Hemsworth', leave:'', dues:'', total:',200', status:'Completed'}];
    ...
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Process final payroll, bonuses, and encashments</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Employee</th><th className='px-6 py-3'>Leave Encashment</th><th className='px-6 py-3'>Pending Dues</th><th className='px-6 py-3'>Total Payout</th><th className='px-6 py-3'>Status</th>
