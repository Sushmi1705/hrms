...
export function ExitChecklist() {
  const data = [{name:'Alex Turner', it:'Cleared', admin:'Pending', finance:'Pending', status:'In Progress'},{name:'Noah Centineo', it:'Cleared', admin:'Cleared', finance:'Cleared', status:'Completed'}];
    ...
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Track departmental clearance (IT, Admin, Finance)</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Employee</th><th className='px-6 py-3'>IT Clearance</th><th className='px-6 py-3'>Admin Clearance</th><th className='px-6 py-3'>Finance Clearance</th><th className='px-6 py-3'>Status</th>
