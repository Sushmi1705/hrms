...
export function AssetRecovery() {
  const data = [{name:'Alex Turner', laptop:'Returned', card:'Pending', corp:'N/A', status:'In Progress'},{name:'Zendaya', laptop:'Returned', card:'Returned', corp:'Returned', status:'Completed'}];
    ...
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Track the return of company property and equipment</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Employee</th><th className='px-6 py-3'>Laptops/Devices</th><th className='px-6 py-3'>Access Cards</th><th className='px-6 py-3'>Corporate Card</th><th className='px-6 py-3'>Status</th>
