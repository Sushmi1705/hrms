...
export function Documentation() {
  const data = [{name:'Alex Turner', rel:'Draft', exp:'Draft', fnf:'Pending', status:'Pending'},{name:'Robert Downey', rel:'Issued', exp:'Issued', fnf:'Issued', status:'Completed'}];
    ...
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Issue relieving letters and experience certificates</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Employee</th><th className='px-6 py-3'>Relieving Letter</th><th className='px-6 py-3'>Experience Cert</th><th className='px-6 py-3'>FnF Statement</th><th className='px-6 py-3'>Status</th>
