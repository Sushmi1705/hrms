...
export function SecurityEvents() {
  ...
  const { data, isLoading, isError, refetch, isFetching } = useQuery({
    queryKey: ['SecurityEvents', page, search],
    queryFn: async () => {
      const res = await fetch(`http://localhost:5002/api/v1/audit/security-events?page=` + page + `&search=` + search);
  ...
  return (
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <CardTitle className="text-lg font-semibold text-slate-800">Security Events</CardTitle>
          <p className="text-sm text-slate-500 mt-1">High-priority alerts for potential unauthorized access and policy violations</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Timestamp</th><th className='px-6 py-3'>Event Type</th><th className='px-6 py-3'>Description</th><th className='px-6 py-3'>IP Address</th><th className='px-6 py-3'>Severity</th>
...
<td className='px-6 py-4 whitespace-nowrap'>{new Date(item.createdAt).toLocaleString()}</td><td className='px-6 py-4 whitespace-nowrap font-medium text-red-700'>{item.eventType}</td><td className='px-6 py-4 max-w-[300px] truncate' title={item.description}>{item.description}</td><td className='px-6 py-4 whitespace-nowrap font-mono text-xs'>{item.ipAddress}</td><td className='px-6 py-4 whitespace-nowrap'>{getStatusBadge(item.severity)}</td>
