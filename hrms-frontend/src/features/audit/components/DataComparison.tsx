  ...
  return (
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <CardTitle className="text-lg font-semibold text-slate-800">Data Revision Diff</CardTitle>
          <p className="text-sm text-slate-500 mt-1">Before and After snapshot of altered Database records</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Timestamp</th><th className='px-6 py-3'>Entity Name</th><th className='px-6 py-3'>Property</th><th className='px-6 py-3'>Old Value</th><th className='px-6 py-3'>New Value</th>
...
<td className='px-6 py-4 whitespace-nowrap'>{new Date(item.createdAt).toLocaleString()}</td><td className='px-6 py-4 whitespace-nowrap font-medium text-indigo-700'>{item.entityName}</td><td className='px-6 py-4 whitespace-nowrap'>{item.propertyName}</td><td className='px-6 py-4 max-w-[200px] truncate font-mono text-xs text-red-600 bg-red-50 p-1 rounded' title={item.oldValue}>{item.oldValue || 'null'}</td><td className='px-6 py-4 max-w-[200px] truncate font-mono text-xs text-green-600 bg-green-50 p-1 rounded' title={item.newValue}>{item.newValue || 'null'}</td>
