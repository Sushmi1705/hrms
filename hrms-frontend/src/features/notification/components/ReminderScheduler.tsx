...
export function ReminderScheduler() {
  ...
  const { data, isLoading, isError, refetch, isFetching } = useQuery({
    queryKey: ['ReminderScheduler', page, search],
    queryFn: async () => {
  ...
  return (
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <CardTitle className="text-lg font-semibold text-slate-800">Cron Reminder Scheduler</CardTitle>
          <p className="text-sm text-slate-500 mt-1">Configure recurring system reminders (Daily, Weekly, Monthly)</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Reminder Name</th><th className='px-6 py-3'>Schedule (Cron)</th><th className='px-6 py-3'>Next Run</th><th className='px-6 py-3'>Status</th>
...
<td className='px-6 py-4 whitespace-nowrap font-medium'>Timesheet Reminder</td><td className='px-6 py-4 whitespace-nowrap font-mono text-xs'>0 17 * * 5</td><td className='px-6 py-4 whitespace-nowrap'>{new Date().toLocaleString()}</td><td className='px-6 py-4 whitespace-nowrap'>{getStatusBadge('Scheduled')}</td>
