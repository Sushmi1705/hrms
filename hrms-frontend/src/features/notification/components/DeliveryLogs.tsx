  ...
  return (
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <CardTitle className="text-lg font-semibold text-slate-800">System Delivery Audit Logs</CardTitle>
          <p className="text-sm text-slate-500 mt-1">Comprehensive log of all sent and failed messages</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Queue ID</th><th className='px-6 py-3'>Channel</th><th className='px-6 py-3'>Recipient</th><th className='px-6 py-3'>Timestamp</th><th className='px-6 py-3'>Status</th>
...
<td className='px-6 py-4 whitespace-nowrap font-mono text-xs'>{item.notificationQueueId.substring(0,8)}...</td><td className='px-6 py-4 whitespace-nowrap font-medium'>{item.channel}</td><td className='px-6 py-4 whitespace-nowrap'>{item.recipient}</td><td className='px-6 py-4 whitespace-nowrap'>{new Date(item.createdAt).toLocaleString()}</td><td className='px-6 py-4 whitespace-nowrap'>{getStatusBadge(item.status)}</td>
