...
export function Announcements() {
  ...
  const { data, isLoading, isError, refetch, isFetching } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`http://localhost:5002/api/v1/notification/announcements?page=` + page + `&search=` + search);
  ...
  const renderSkeleton = () => (
    [1, 2, 3, 4, 5].map((i) => (
      <tr key={i} className="animate-pulse">
        <td className="px-6 py-4"><div className="h-4 bg-slate-200 rounded w-3/4"></div></td> <td className="px-6 py-4"><div className="h-4 bg-slate-200 rounded w-3/4"></div></td> <td className="px-6 py-4"><div className="h-4 bg-slate-200 rounded w-3/4"></div></td> <td className="px-6 py-4"><div className="h-4 bg-slate-200 rounded w-3/4"></div></td> <td className="px-6 py-4"><div className="h-4 bg-slate-200 rounded w-3/4"></div></td>
  ...
  return (
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <CardTitle className="text-lg font-semibold text-slate-800">Company Announcements</CardTitle>
          <p className="text-sm text-slate-500 mt-1">Broadcast messages to specific departments or the entire organization</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Title</th><th className='px-6 py-3'>Message</th><th className='px-6 py-3'>Audience</th><th className='px-6 py-3'>Date</th><th className='px-6 py-3'>Status</th>
...
<td className='px-6 py-4 whitespace-nowrap font-medium'>{item.title}</td><td className='px-6 py-4 line-clamp-1'>{item.message}</td><td className='px-6 py-4 whitespace-nowrap'>{item.targetAudience}</td><td className='px-6 py-4 whitespace-nowrap'>{new Date(item.createdAt).toLocaleDateString()}</td><td className='px-6 py-4 whitespace-nowrap'>{getStatusBadge(item.isPinned ? 'Active' : 'Unpinned')}</td>
