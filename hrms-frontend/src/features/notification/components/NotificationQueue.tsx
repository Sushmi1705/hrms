...
export function NotificationQueue() {
  ...
  const { data, isLoading, isError, refetch, isFetching } = useQuery({
    queryKey: ['NotificationQueue', page, search],
    queryFn: async () => {
      const res = await fetch(`http://localhost:5002/api/v1/notification/queue?page=` + page + `&search=` + search);
  ...
  return (
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <CardTitle className="text-lg font-semibold text-slate-800">Live Processing Queue</CardTitle>
          <p className="text-sm text-slate-500 mt-1">Monitor background processing of scheduled communications</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Queue ID</th><th className='px-6 py-3'>Channel</th><th className='px-6 py-3'>Payload</th><th className='px-6 py-3'>Status</th>
...
<td className='px-6 py-4 whitespace-nowrap font-mono text-xs'>{item.id}</td><td className='px-6 py-4 whitespace-nowrap font-medium'>{item.channel}</td><td className='px-6 py-4 text-xs font-mono text-slate-500'>{item.payload}</td><td className='px-6 py-4 whitespace-nowrap'>{getStatusBadge(item.status)}</td>
