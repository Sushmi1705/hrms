...
export function PushTemplates() {
  ...
  const { data, isLoading, isError, refetch, isFetching } = useQuery({
    queryKey: ['PushTemplates', page, search],
    queryFn: async () => {
      const res = await fetch(`${API_BASE_URL}/api/v1/notification/templates?page=` + page + `&search=` + search);
  ...
  return (
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <CardTitle className="text-lg font-semibold text-slate-800">Web & Mobile Push Templates</CardTitle>
          <p className="text-sm text-slate-500 mt-1">Short and actionable push notification logic</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Template Name</th><th className='px-6 py-3'>Push Title</th><th className='px-6 py-3'>Push Body Preview</th><th className='px-6 py-3'>Status</th>
...
<td className='px-6 py-4 max-w-[200px] truncate font-medium' title={item.name}>{item.name}</td><td className='px-6 py-4 max-w-[200px] truncate' title="Action Required">Action Required</td><td className='px-6 py-4 max-w-[300px] truncate' title="Please approve request.">Please approve request.</td><td className='px-6 py-4 whitespace-nowrap'>{getStatusBadge('Active')}</td>
