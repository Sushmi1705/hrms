...
export function NotificationSettings() {
  ...
  const { data, isLoading, isError, refetch, isFetching } = useQuery({
    queryKey: ['NotificationSettings', page, search],
    queryFn: async () => {
  ...
  return (
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <CardTitle className="text-lg font-semibold text-slate-800">Notification Provider Settings</CardTitle>
          <p className="text-sm text-slate-500 mt-1">Configure API keys and Global Platform Limits</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Setting Category</th><th className='px-6 py-3'>Key</th><th className='px-6 py-3'>Value</th><th className='px-6 py-3'>Status</th>
...
<td className='px-6 py-4 whitespace-nowrap font-medium'>Email Provider</td><td className='px-6 py-4 whitespace-nowrap'>SMTP_HOST</td><td className='px-6 py-4 whitespace-nowrap'>smtp.sendgrid.net</td><td className='px-6 py-4 whitespace-nowrap'>{getStatusBadge('Active')}</td>
