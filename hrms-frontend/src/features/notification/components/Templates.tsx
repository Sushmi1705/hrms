  ...
  return (
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <CardTitle className="text-lg font-semibold text-slate-800">Notification Templates</CardTitle>
          <p className="text-sm text-slate-500 mt-1">Manage cross-channel notification logic and placeholders</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Template Name</th><th className='px-6 py-3'>Category</th><th className='px-6 py-3'>Variables</th><th className='px-6 py-3'>Status</th>
...
<td className='px-6 py-4 whitespace-nowrap font-medium'>{item.name}</td><td className='px-6 py-4 whitespace-nowrap'>{item.category}</td><td className='px-6 py-4 font-mono text-xs text-slate-500'>{item.variables}</td><td className='px-6 py-4 whitespace-nowrap'>{getStatusBadge(item.isActive ? 'Active' : 'Draft')}</td>
