...
export function EmailTemplates() {
  ...
  const { data, isLoading, isError, refetch, isFetching } = useQuery({
    queryKey: ['EmailTemplates', page, search],
    queryFn: async () => {
  ...
  return (
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <CardTitle className="text-lg font-semibold text-slate-800">Rich HTML Email Templates</CardTitle>
          <p className="text-sm text-slate-500 mt-1">Build and customize company-branded email designs</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Template Name</th><th className='px-6 py-3'>Subject Line</th><th className='px-6 py-3'>Last Updated</th><th className='px-6 py-3'>Status</th>
...
<td className='px-6 py-4 max-w-[200px] truncate font-medium' title={item.name}>{item.name}</td><td className='px-6 py-4 max-w-[300px] truncate' title="Auto-generated Sub">Auto-generated Sub</td><td className='px-6 py-4 whitespace-nowrap'>{new Date().toLocaleDateString()}</td><td className='px-6 py-4 whitespace-nowrap'>{getStatusBadge('Active')}</td>
