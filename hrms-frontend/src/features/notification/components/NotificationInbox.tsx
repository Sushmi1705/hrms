...
export function NotificationInbox() {
  ...
  const { data, isLoading, isError, refetch, isFetching } = useQuery({
    queryKey: ['NotificationInbox', page, search],
    queryFn: async () => {
      const res = await fetch(`http://localhost:5002/api/v1/notification/inbox?page=` + page + `&search=` + search);
  ...
  return (
    <Card className="shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <CardTitle className="text-lg font-semibold text-slate-800">Universal Notification Inbox</CardTitle>
          <p className="text-sm text-slate-500 mt-1">View and manage all notifications received across the system</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
<th className='px-6 py-3'>Module</th><th className='px-6 py-3'>Type</th><th className='px-6 py-3'>Title</th><th className='px-6 py-3'>Message</th><th className='px-6 py-3'>Date</th><th className='px-6 py-3'>Status</th>
...
<td className='px-6 py-4 whitespace-nowrap font-medium text-slate-900'>{item.module}</td><td className='px-6 py-4 whitespace-nowrap'>{getStatusBadge(item.type)}</td><td className='px-6 py-4 whitespace-nowrap'>{item.title}</td><td className='px-6 py-4 text-slate-500'>{item.message}</td><td className='px-6 py-4 whitespace-nowrap'>{new Date(item.createdAt).toLocaleString()}</td><td className='px-6 py-4 whitespace-nowrap'>{item.isRead ? getStatusBadge('Read') : getStatusBadge('Unread')}</td>
